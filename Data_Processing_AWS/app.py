...
def user_input():
    if request.method == 'POST':
        res_json = json.loads(request.data)
        data = df[(df['gu'] == (f'{res_json["region_gu"][0]}')) & (
            df['dong'] == (f'{res_json["region_dong"][0]}'))][['lat', 'lng', 'code']]
        d_records = data.to_dict('records')[0]
        d_records['region_gu'] = res_json["region_gu"][0]
        d_records['region_dong'] = res_json["region_dong"][0]
        d_records['service'] = res_json["service"][0]
        ...
        data = bus_stop_df[(bus_stop_df['gu'].str.contains(f'{res_json["region_gu"][0]}'[:1])) & (
            bus_stop_df['dong'].str.contains(f'{res_json["region_dong"][0]}'[:1]))][['xcode', 'ycode']]
